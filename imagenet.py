...
class ILSVRC(torch.utils.data.Dataset):
    def __init__(self,ilsvrc_data_path,meta_path,transform=None,val=False):
    ...
    def __getitem__(self,index):
        ...
        label=example['label']
        img = self.open_image(example['image_path'])
    ...
    def preprocess_image(self,image_path):
        ...
        IMAGENET_STD = [58.34, 57.12, 57.38]
...
if __name__=='__main__':
    ...
    for img_, label_ in train_loader:
        if torch.any(torch.isnan(img_)):
            print(img_, label_)
